...
<html>
	<head>
		<t:head />
		<script language="javascript" src="html/js/base64.js"></script>
		<script language="javascript">
			...
			function checkName() {
				...
				if (userName.length == 0) {
					...
					if(flag) {
						 //对数据加密  
						var encryUsername = encode64(userName); 
			            var encryPassword = encode64(password);  
			         	// 加密用户名及密码
			            document.form1.userName.value = encryUsername;  
			            document.form1.password.value = encryPassword;  
			            flag = false;
						form1.submit();
					}else{
						window.alert("正在登录中，请稍等！");	
					}
				}
			}
		</script>
	...
	<body>
		...
		<div class="content">
			<FORM name=form1 action="login.do" method="POST">
				<table width="820" border="0" cellSpacing="3" cellPadding="0"
					bgColor="#E0FCDE">
					<c:if test="${empty user and empty students}">
						<!-- login页面-->
						<tr height="10">
							<td colspan="3">&nbsp;
					...
					<c:if test="${not empty user || not empty students}">
			...
			<table cellSpacing="0" cellPadding="6" width="820" border="1">
				<tr>
